...
{load_project_tasklist assign="project_tasklist"}
    ...
    <div class="post" style="margin-right:225px;">
      <h3><strong>工作清單</strong></h3>
      <blockquote style="margin:0;">
        {if $project_tasklist->mProjectTaskList}
        <ol>
          {section name=i loop=$project_tasklist->mProjectTaskList}
          <li><a href="main.php?event=Task&action=ViewList&id={$project_tasklist->mProjectTaskList[i].id}&active_project={$smarty.get.active_project}">{$project_tasklist->mProjectTaskList[i].name}</a></li>
          {/section}
        </ol>
        {else}
