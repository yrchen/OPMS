<!-- Begin of apply-project-leave.tpl -->
...
{if $apply_project_content->mApplyProjectContent}
    ...
    <div class="post">
      ...
      {if !$apply_project_content->mApplyExistFlag}
        <h3><strong>已經取消報名囉！</strong></h3>
      {else}
        <h3><strong>確定取消報名？</strong></h3>
        <form name="project_edit" action="main.php?event=Apply&action=LeaveProject&id={$apply_project_content->mApplyProjectContent.id}" method="post">
          <input type="submit" name="submit_leave_project_confirm_{$apply_project_content->mApplyProjectContent.id}" class="button" value="取消報名">
...
{/if}
<!-- End of apply-project-leave.tpl -->